...
if api_call GET "$JS_URL/api/v1/settings/public"; then
  ...
  if [ "$JS_INIT" = "True" ]; then
    ...
    if api_call_cookie POST "$JS_URL/api/v1/auth/jellyfin" \
      "{\"username\":\"${JELLYFIN_USER:-admin}\",\"password\":\"${JELLYFIN_PASSWORD:-admin}\",\"hostname\":\"http://jellyfin:8096\",\"email\":\"admin@media.server\"}" \
      "$JS_COOKIES"; then
    ...
    api_call_cookie POST "$JS_URL/api/v1/settings/jellyfin" \
      '{"name":"Jellyfin","hostname":"jellyfin","port":8096,"useSsl":false,"urlBase":""}' \
